...
The model takes messages as input and outputs messages after generating a complete response.](#invoke)
[## Stream

Invoke the model, but stream the output as it is generated in real-time.](#stream)
[## Batch

Send multiple requests to a model in a batch for more efficient processing.](#batch)
